...
# Enable performance optimizations for CPU
#torch.backends.cpu.enabled = True
#torch.set_num_threads(4)  # Adjust based on your CPU cores
#torch.set_num_interop_threads(1)  # Helps with memory usage
...
def greedy_decode(model, source, source_mask, tokenizer_src, tokenizer_tgt, max_len, device):
    ...
    while True:
        ...
        # Select the token with the maximum probability => because of greedy search
...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_state, writer, num_examples = 2):
    ...
    #with torch.no_grad(): # No gradient calculation for validation dataset
        #for batch in validation_ds:
            #count += 1
...
def get_ds(config):
    ...
    # Enable faster data loading with multiple workers
...
def train_model(config):
    ...
    # Use target tokenizer's PAD token for ignore_index
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            # Step optimizer only after accumulating gradients for specified steps
